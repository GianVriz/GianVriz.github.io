...
```{r, echo = FALSE, results = 'hide', include = TRUE, eval = FALSE}
#Note:The following chunk can also be used to build the dataset. Ensure the working directory is properly set before importing the data when running from the console.
##RENIXX
...
set.seed(004)
# Force single-threaded MCMC and BLAS
options(mc.cores = 1)
Sys.setenv(
  OMP_NUM_THREADS = "1",
...
for (i in 73:(sample - 1)) {
  ...
  #Saving
  predictions_MSCI_GAE[(i - 73 + 1), 2] <- pred_bayesian2$mean
}
#
#BSTS iShares_GCE
for (i in 73:(sample - 1)) {
  ...
  #Saving
  predictions_iShares_GCE[(i - 73 + 1), 2] <- pred_bayesian2$mean
}
#
#BSTS SP_GCE
for (i in 73:(sample - 1)) {
  ...
  #Saving
  predictions_SP_GCE[(i - 73 + 1), 2] <- pred_bayesian2$mean
}
```
